...
# Person class. Skeleton of other 'people' classes(doctor, nurse & patient).
class Person:
...
class Nurse(Person):
    ...
    def create_patient(self): # Prompts several info to make a new patient
        print("Ready to create patient. Please enter the following details:")

        new_id = len(patients) + 1
        new_first_name = input("[1/5] Enter patient first name: ").strip()
        new_last_name = input("[2/5] Enter patient last name: ").strip()

        try:
            new_age = int(input("[3/5] Enter patient age: ").strip())
        except ValueError:
            print("Age must be a number.")
            return

        new_admit_date = input("[4/5] Enter patient's admittance date (Month, Day): ").strip()
        new_address = input("[5/5] Enter patient address: ").strip()
    ...
    def view_patient(self): #views patient general info
        try:
            view_id = int(input("Please enter patient ID for details: "))
        except ValueError:
...
# Class of all the patients and their data
class Patient(Person):
    ...
    def retrieve_patient_info(self):
        ...
        print("Age:", self.age)
        print("Admit Date:", self.admit_date)
        print("Address:", self.address)
        print("Doctor:", self.doctor)
        print("Medical Costs:", self.med_costs)
...
# This function searches the patient list and returns the patient object with the matching ID.
...
def login():
    ...
    for doctor in doctors:
        if doctor.get_doctor_id().lower() == doc_id.lower() and doctor.get_password() == password:
            print("Login successful. Welcome to the HIS")
...
def create_patient():
    ...
    try:
        patient_med_costs = float(input("Enter patient medical costs: ").strip())
    except ValueError:
        print("Medical costs must be a number.")
        return
...
# This function finds a patient and adds a lab test
# It also updates the total medical cost
def order_lab_test():
    ...
    for patient in patients:
        if patient.first_name == patient_name:
            ...
            try:
                cost = float(input("Enter test cost: ").strip())
            except ValueError:
                print("Test cost must be a number.")
                return

            patient.lab_tests.append((test_name, cost))
            patient.med_costs += cost

            print("Lab test ordered successfully.")
            return 

    print("patient not found.")

# View patients function:
# This function displays all patients details (name, age, etc)
...
def run_patient():
    ...
    while True:
        ...
        if choice == 1:
            ...
            if patient_id.patient_id in med_manager.patient_meds:
                for med in med_manager.patient_meds[patient_id.patient_id]:
                    print(med)
            else:
                print("No medications found.")
        elif choice == 3:
            if patient_id.patient_id in vitals_manager.patient_vitals:
                for vital in vitals_manager.patient_vitals[patient_id.patient_id]:
                    print(vital.display())
            else:
...
class MedicationManager:
    ...
    def prescribe_med(self):
        ...
        patient = find_patient_by_id(view_id)
        if patient is not None:
            patient.medications = self.patient_meds[view_id]
...
class VitalsManager:
    ...
    def add_vital(self):
       # Checks to see if user is logged in as a doctor or nurse
        ...
        patient = find_patient_by_id(view_id)
        if patient is not None:
            patient.vitals = self.patient_vitals[view_id]
...
class Print:
    ...
    def print_patient(self):
        ...
        patient_info = (
            ...
            f"Lab Tests: {self.patient.lab_tests if self.patient.lab_tests else 'None'}\n"
            f"Vitals: {self.patient.vitals if self.patient.vitals else 'None'}\n"       
            f"Medications: {self.patient.medications if self.patient.medications else 'None'}\n"
        )
...
# This line of code initiates the program
main_module()
